...
/*   Updated: 2025/09/15 08:54:46 by jpuerto-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/cub3d.h"

void	ft_draw_line(t_player *player, t_game *game, float camera_x, int i)
{
	t_draw_data	d;

	ft_init_line_data(&d, player, game, camera_x);
	ft_calc_wall_position(&d, player, game);
	ft_calc_texture_data(&d, game);
	ft_draw_wall_column(&d, game, i);
}

void	ft_init_line_data(t_draw_data *d, t_player *player, t_game *game, float camera_x)
{
	d->l = ft_init_line(player, camera_x);
	ft_calculate_steps(&d->l, player);
	ft_dda(game, &d->l);
	d->l.dist = ft_get_dist(player, d->l); // Asegúrate de usar la distancia perpendicular y en bloques
	d->height = (BLOCK * HEIGHT) / d->l.dist; // Sin SCALE_BLOCK
